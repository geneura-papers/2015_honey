...
\begin{itemize}
\item Vector, which will be single or multiple according to the number of simultaneous vulnerabildiades that the cyberthreat explodes.
\item Persistence, if the cyberthreat is continuous in time and establishes monitoring and control mechanisms with hostile agent.
\item Advanced, if the cyberthreat used mechanisms in order to hide its activity in the system.
\end{itemize}

One of the greatest dangers for IT infrastructure of governments, public administrations and companies are advance persistence threats. Usually APT are related with ciberspying and elite groups of cibercrime and they are attacks directed against specific infrastructure. For this reason it is a priority to detect and study them.\\
...
%ESTADISTICAS DEL PFC

\section{Strengths and Weaknesses of Honeypots}

The strengths of the deployed system were:
\begin{itemize}
\item It was not intrusive network traffic, remained the privacy of infrastructure's users.
\item The computational and economic resources needed  for passive detection are lower because we have only the traffic belong to a potential cyberthreat.
\item When the communication between the sensor and external infrastructure that supports the cyberthreat is ciphered, it is essential catch the information from the application layer.
\end{itemize}

The weakness of the deployed system were:
\begin{itemize}
\item There are many cyberthreats focused in the network layer, usually related with deny of services and spoofing. And this information is very valuable because this kind of attack are a very important element in both simple and complex attacks.
\item Like others deception tools, honeypots must show itself like interesting target for a attacker and avoid to be easily recognizable by fingerprint techniques %REFERENCIA AL ARTICULO DE PABLO GONZALES
\end{itemize}
