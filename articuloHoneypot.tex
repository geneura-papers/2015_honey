...
When we look at the clusters, some attacks to services built more defined groups than others. The attacks to web servers (HTTP protocol), Microsoft SQL Server (MsSQL) and Session Initiation Protocol are grouped in more reduced areas than others like Samba (SMB)......xxxxxxxxxxxxxxxxxxx. This could be interpreted like a vector with components more similar to each other so we interpret that the behavior of the cybertreats is more common. On the other hand attacks to SMB or MySQL, it shows more variety in somes dimensions of its vector, generating more extended clusters. In the last case this characteristic is due to some attacks
